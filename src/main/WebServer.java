...
public class WebServer {
	...
	/**
	 * 	http://localhost:8000/get?key=X
	 * GET request used for key lookup. The only request parameter should be
	 * key=X where X is the name of the key. 
	 * 
	...
	static class GetHandler implements HttpHandler {
		public void handle(HttpExchange httpExchange) throws IOException {
			Map<String, String> parms = WebServer.queryToMap(httpExchange.getRequestURI().getQuery());
	...
	/**
	 * 	http://localhost:8000/set?key=X&value=Y
	 * GET request used for key insertion into the redis db. The request parameters should be
	 ...
	 */	static class SetHandler implements HttpHandler {
		public void handle(HttpExchange httpExchange) throws IOException {
			...
			LRUCache.getInstance().set(key, value);
			writeResponse(httpExchange, "Successfully added " + key + "/" + value);
		}
	}
